...
public class Mesh {
    ...
    protected Vector<Face> F;
    protected Vector<Vert> V;
	...
	Edge sym = E.get(edge.sym);
    	...
    	// set up v0
    	//Vec3 vprev = V.get(edge.v).coord;
    	//Vec3 vnext = V.get(E.get(edge.next).v).coord;
    	// XXX v0.coord = Vec3.add(vprev, vnext).scale(0.5f);
    	v0.coord = newPos;
    	v0.i = V.size();
    	v0.e = e0.i;
    	V.add(v0);
    ...
    protected Hashtable<Integer,Vec3> oddLoopVerts() {
    	...
    	Vector<Integer> symEdges = new Vector<Integer>();
    	for(int i = 0; i < F.size(); i++){
    		Edge firstEdge = E.get(F.get(i).e);
    		Edge nextEdge = firstEdge;
    		do{
    			if(!symEdges.contains(nextEdge.i)){
        			// current edge indexed for this face hasn't been split yet
        			// get the 4 relative vertices, then scale and combine
        			Vec3 v1 = (Vec3) V.get(nextEdge.v).coord.clone();
        			Vec3 v2 = (Vec3) V.get(E.get(nextEdge.next).v).coord.clone();
        			Vec3 v3 = (Vec3) V.get(E.get(nextEdge.prev).v).coord.clone();
        			Vec3 v4 = (Vec3) V.get(E.get(E.get(nextEdge.sym).prev).v).coord.clone();
        			v1 = v1.scale(3.0f/8.0f);
        			v2 = v2.scale(3.0f/8.0f);
        			v3 = v3.scale(1.0f/8.0f);
        			v4 = v4.scale(1.0f/8.0f);
        			oddVerts.put(nextEdge.i, Vec3.add(v1, Vec3.add(v2, Vec3.add(v3,v4))));
        		}
    			// add sym edge index to Vector list so we don't try
    			// to split the same edge twice later on
    			// since splitEdge handles edge + edge.sym at once
    			symEdges.add(nextEdge.sym);
    			nextEdge = E.get(nextEdge.next);
    		}while(nextEdge.i != firstEdge.i);
    	}
    	return oddVerts;
    }
